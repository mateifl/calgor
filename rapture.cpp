...
vector<ulong> bellman_ford(vector<edge> &g, ulong l_nodes_number,  ulong l_source_node) {
	...
	for (ulong i = 0; i < l_nodes_number - 1; i++)
	{
		for(ulong j = 0; j < g.size(); j++)
		{
            ...
            long i_updated_value = v_shortest_path[e.head] - e.value;
            if(i_updated_value < 0)
                i_updated_value = 0; 
			if (v_shortest_path[e.head] + i_updated_value < v_shortest_path[e.tail])
				v_shortest_path[e.tail] = v_shortest_path[e.head] + i_updated_value;
            i_updated_value = v_shortest_path[e.tail] - e.value;
            if(i_updated_value < 0)
                i_updated_value = 0;
			if (v_shortest_path[e.tail] + i_updated_value < v_shortest_path[e.head])
				v_shortest_path[e.head] = v_shortest_path[e.tail] + i_updated_value;
